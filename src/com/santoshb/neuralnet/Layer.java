...
public class Layer {
	...
	public Layer(int inputSize, int outputSize) {
		...
		// why +1 - Because we need one additional node for the Bias NODE
		input = new float[inputSize + 1];
		// Weights would be from every input neurons to every output hidden neurons or output so input * output
	...
	public float[] train(float[] error, float learningRate, float momentum) {
		...
		for (int i = 0; i < output.length; i++) {
			...
			// because the output is the sigmoid(x) !!! 
